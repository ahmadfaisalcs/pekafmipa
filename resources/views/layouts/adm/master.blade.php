...
<head>
  ...
  <!-- bootstrap datatable -->
  <link rel="stylesheet" href="{{ asset('public/adminlte/plugins/datatables/dataTables.bootstrap.css') }}" type="text/css">
...
<div class="wrapper">
  ...
  <!-- Control Sidebar -->
  {{-- @include('layouts.adm.control-sidebar') --}}
  <!-- /.control-sidebar -->
...
<script>
  var allcheck = 0;
  var keterangan = getCheckedBoxes("cek");
  if( $('.cek:checked').length != $('.cek').length ) {
    document.getElementById("keterangan").value = keterangan;
  }

  function getCheckedBoxes(chkbox) {
    var checkboxes = document.getElementsByClassName(chkbox);
    var checkboxesChecked = [];
    var keterangan = "";
    var prefix = "File lampiran ";
    var endword = "salah, silahkan diperbarui";
    ...
    for (var j=0; j<checkboxesChecked.length; j++) {
      keterangan = keterangan.concat(checkboxesChecked[j].value);
      if (checkboxesChecked.length-j == 1) {
        keterangan = keterangan.concat(" ");
      }
      else {
        keterangan = keterangan.concat(",");
        keterangan = keterangan.concat(" ");
    ...
    keterangan = prefix.concat(keterangan);
    keterangan = keterangan.concat(endword);
    return checkboxesChecked.length > 0 ? keterangan : null;
  }
  ...
  $(".cek").change(function(){
    ...
    if( $('.cek:checked').length == $('.cek').length )
    {
      ...
      document.getElementById("is_allchecked").value = "1";
    }
    else 
    {
      allcheck = 0;
      // console.log(checkedBoxes);
      document.getElementById("is_allchecked").value = "0";
      document.getElementById("keterangan").value = keterangan;
    }
  });
  // console.log(checked);
  ...
  $('.tombol').click(function(){
    ...
    if (allcheck) {
      $('#berkaslengkap').modal('show');
    }
    else {
      $('#berkastdklengkap').modal('show');
    }
  });
</script>
</body>
